...
export default function Dashboard() {
  ...
  if (!dashboard) {
    return <p className="text-red-500">{error || 'No dashboard data available.'}</p>;
  }
  ...
  const totalLoans = activeLoans.reduce((sum: number, loan: any) => sum + loan.outstandingBalance, 0);
  const monthlyGrowth = calculateMonthlyGrowth(transactions);

  const quickActions = [
    { name: 'Deposit Money', href: '/deposit', icon: Plus, color: 'bg-green-600 hover:bg-green-700', description: 'Add funds to your account' },
    { name: 'Transfer Money', href: '/transfer', icon: ArrowUpRight, color: 'bg-blue-600 hover:bg-blue-700', description: 'Send money to other accounts' },
    { name: 'Withdraw Cash', href: '/withdraw', icon: ArrowDownLeft, color: 'bg-orange-600 hover:bg-orange-700', description: 'Withdraw from your account' },
    { name: 'Pay Bills', href: '/bills', icon: Receipt, color: 'bg-purple-600 hover:bg-purple-700', description: 'Utilities, credit cards, and more' }
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 via-blue-700 to-blue-900 rounded-xl p-6 text-white shadow-lg">
        ...
        <div className="space-y-1">
          <p className="text-blue-100">Account: {primaryAccount?.accountNumber}</p>
          <p className="text-2xl font-bold">${dashboard.totalBalance?.toLocaleString() || '0.00'}</p>
          ...
          {transactions.length === 0 ? (
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto">
              {transactions.map((tx: any) => {
                const isCredit = ['deposit', 'transfer_in', 'loan_disbursement'].includes(tx.transactionType);
                return (
                  <div key={tx._id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
...
function calculateMonthlyGrowth(transactions: any[]): string {
  ...
  const credits = thisMonth
    .filter(tx => ['deposit', 'transfer_in', 'loan_disbursement'].includes(tx.transactionType))
    .reduce((sum, tx) => sum + tx.amount, 0);

  const debits = thisMonth
    .filter(tx => ['withdrawal', 'transfer_out', 'bill_payment'].includes(tx.transactionType))
    .reduce((sum, tx) => sum + tx.amount, 0);
