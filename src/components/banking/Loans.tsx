...
function RepaymentModal({
  ...
  const handleRepay = async (e: React.FormEvent) => {
    ...
    if (amount > loan.outstandingBalance) {
      setError(
        `Amount cannot exceed outstanding balance of ₦${loan.outstandingBalance.toLocaleString()}`
      );
      return;
    }

    const selectedAcc = accounts.find((a) => a._id === repayAccount);
    ...
    setLoading(true);
    try {
      await loanAPI.repay({ loanId: loan._id, amount, accountId: repayAccount });
      setDone(true);
      setTimeout(() => {
        onSuccess();
        onClose();
      ...
      setError(err.response?.data?.error || "Repayment failed. Please try again.");
    } finally {
      setLoading(false);
    }
  };
        ...
        {done ? (
          <div className="text-center py-6">
            ...
            <p className="text-gray-500 text-sm">
              ₦{parseFloat(repayAmount).toLocaleString()} has been applied to your loan.
            </p>
          </div>
        ) : (
          <>
            <div className="flex items-center mb-5">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-900">Repay Loan</h3>
            ...
            <form onSubmit={handleRepay} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    { label: "Min. payment", value: loan.monthlyPayment },
                    { label: "Half balance", value: +(loan.outstandingBalance / 2).toFixed(2) },
                    { label: "Full balance", value: loan.outstandingBalance },
                  ].map((opt) => (
                    <button
                      key={opt.label}
                      type="button"
                      onClick={() => setRepayAmount(opt.value.toString())}
                      className="px-3 py-1 text-xs border border-gray-300 rounded-md hover:bg-purple-50 hover:border-purple-400 transition"
                    >
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={onClose}
                  className="flex-1 border border-gray-300 text-gray-700 py-2 rounded-md hover:bg-gray-50 transition text-sm font-medium"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 bg-purple-600 text-white py-2 rounded-md hover:bg-purple-700 transition disabled:opacity-50 text-sm font-medium"
                >
...
export default function Loans() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (accountsData.length > 0) setSelectedAccount(accountsData[0]._id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (amount <= 0) { setError("Please enter a valid loan amount"); return; }
    if (amount < 10000) { setError("Minimum loan amount is ₦10,000"); return; }
    if (months < 6 || months > 60) { setError("Loan term must be between 6 and 60 months"); return; }
    if (monthlyIncome <= 0) { setError("Please enter a valid monthly income"); return; }
    if (!selectedAccount) { setError("Please select an account"); return; }
    if (!purpose.trim()) { setError("Please provide a loan purpose"); return; }
    ...
    setLoading(true);
    try {
      await loanAPI.apply({ accountId: selectedAccount, loanAmount: amount, termMonths: months });
      setSuccess(true);
      setLoanAmount("");
      setTermMonths("12");
      setPurpose("");
      ...
      <div className="max-w-md mx-auto">
        <div className="bg-white rounded-lg shadow-lg p-8 text-center">
          ...
          <p className="text-gray-600 mb-4">
            Your loan of ₦{parseFloat(loanAmount).toLocaleString()} has been approved and disbursed
            to your account. You'll receive a confirmation email shortly.
          </p>
                  ...
                  <div className="bg-gradient-to-br from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-5">
                    ...
                    <div className="space-y-2 text-sm border-t border-purple-200 pt-3">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">₦{parseFloat(totalInterest).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Total Payment:</span>
                        <span className="font-medium">₦{parseFloat(totalPayment).toLocaleString()}</span>
                      </div>
                        ...
                        <span
                          className={`font-semibold ${
                            parseFloat(debtToIncomeRatio) > 40
                              ? "text-red-600"
                              : parseFloat(debtToIncomeRatio) > 30
                              ? "text-orange-600"
                              : "text-green-600"
                          }`}
                        >
                    ...
                    {parseFloat(debtToIncomeRatio) > 40 && (
                      <div className="mt-3 flex items-start bg-red-50 border border-red-200 rounded p-3">
                        ...
                        <p className="text-xs text-red-800">
                          Your debt-to-income ratio exceeds 40%. Consider reducing the loan amount
                          or increasing the term.
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <div className="flex items-start">
                    ...
                    <div className="text-sm text-blue-800">
                      <strong>Important:</strong> Loan approval is instant for amounts under
                      ₦1,000,000. Larger loans may require additional verification. Funds will be
                      disbursed immediately upon approval.
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3 max-h-[36rem] overflow-y-auto">
                  {existingLoans.map((loan) => (
                    ...
                    >
                      ...
                      {/* Repay Button — only for active loans */}
                      {loan.status === "active" && (
                        <button
                          onClick={() => setRepayingLoan(loan)}
                          className="mt-3 w-full bg-purple-600 text-white text-xs py-2 rounded-md hover:bg-purple-700 transition font-medium"
                        >
                          Repay Loan
                        </button>
                      )}
